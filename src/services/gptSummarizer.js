...
class GPTSummarizerService {
  ...
  canMakeCall() {
    ...
    // In force mode, allow more GPT calls for testing
    const maxCalls = forceMode ? (this.runsPerUpdate * 2) : this.runsPerUpdate;
    return this.enabled && this.callsUsed < maxCalls;
  }
  ...
  async makeGPTCall(systemPrompt, userPrompt) {
    ...
    try {
      ...
      // Parse bullets from response - handle both single and multi-line formats
      let bullets = [];
      
      // First try to detect numbered list format (1. item, 2. item, etc)
      ...
      if (hasNumberedItems) {
        // Parse as numbered list - each numbered item is a complete bullet
        let currentBullet = '';
        ...
        for (const line of lines) {
          if (numberedPattern.test(line)) {
            ...
            // Remove the number prefix and start new bullet
            currentBullet = line.replace(numberedPattern, '').trim();
          } else if (currentBullet) {
            // Continuation of current bullet (for multi-line items)
            currentBullet += ' ' + line;
          }
        }
      ...
      // Final filtering - remove very short or invalid entries
      bullets = bullets.filter(bullet => bullet.length > 10);

      // Enhanced logging with token details
      const tokensUsed = usage ? `${usage.prompt_tokens}→${usage.completion_tokens}` : 'unknown';
      const forceMode = process.env.GPT_FORCE_MODE === 'true';
      const maxCalls = forceMode ? (this.runsPerUpdate * 2) : this.runsPerUpdate;
      console.log(`   ✅ GPT call ${this.callsUsed}/${maxCalls}: ${bullets.length} bullets, tokens: ${tokensUsed}${forceMode ? ' [FORCE]' : ''}`);
      
      // Log the actual GPT response for debugging in force mode
      if (forceMode && bullets.length > 0) {
        console.log(`   📝 GPT generated: ${bullets.slice(0, 2).map(b => b.substring(0, 50)).join('; ')}...`);
      }
...
${JSON.stringify(trimmedExcerpts, null, 2)}`;
  ...
  /**
   * Summarize breaking news with sparse auto-trigger and organize pass
   * @param {Excerpt[]} excerpts - Array of excerpts
   * @param {string} dateISO - Date in ISO format
   ...
   */
  async summarizeBreaking(excerpts, dateISO, otherCategories = {}) {
    if (!this.enabled) return [];
    
    // Check for sparse content - auto-trigger GPT for breaking when other categories are low
    const sparseThreshold = parseInt(process.env.GPT_SPARSE_THRESHOLD || '3');
    const shouldAutoTrigger = this.shouldTriggerSparseBreaking(otherCategories, sparseThreshold);
    
    if (shouldAutoTrigger) {
      console.log('🎯 Sparse content detected - auto-triggering GPT for breaking news enhancement');
    }
    
    const trimmedExcerpts = this.prepareExcerpts(excerpts, shouldAutoTrigger ? 8 : 5);
    if (trimmedExcerpts.length === 0) return [];

    const systemPrompt = `You are an NFL update assistant. Use ONLY the provided excerpts as ground truth. Do NOT invent facts. If a detail is not stated, omit it. Return short factual bullets (1–2 sentences, ≤ 280 chars) shaped like '<Player or TEAM> — <status/action> (<SOURCE>)'. No URLs. No bylines. No speculation.`;

    const userPrompt = shouldAutoTrigger ? 
      `Category: BREAKING (SPARSE MODE - be more inclusive). Extract ANY significant NFL news not already in injuries/roster: major announcements, extensions, suspensions, coaching changes, official statements, returns, trades, signings. Include team news and player updates.

Excerpts:
${JSON.stringify(trimmedExcerpts, null, 2)}` :
      `Category: BREAKING. Major announcements not already in injuries/roster (extensions, suspensions, official statements, returns). Omit opinion columns.
...
${JSON.stringify(trimmedExcerpts, null, 2)}`;
    ...
    // If sparse mode triggered, run organize pass to improve formatting
    if (shouldAutoTrigger && bullets.length > 0) {
      return this.organizeBreakingBullets(bullets);
    }
  ...
  /**
   * Check if sparse breaking trigger should activate
   * @param {Object} otherCategories - Other category data
   * @param {number} threshold - Sparse threshold
   * @returns {boolean} True if should trigger
   */
  shouldTriggerSparseBreaking(otherCategories, threshold) {
    const injuryCount = otherCategories.injuries?.totalCount || 0;
    const rosterCount = otherCategories.roster?.totalCount || 0;
    const totalOtherContent = injuryCount + rosterCount;
    
    return totalOtherContent <= threshold;
  }
  
  /**
   * Organize breaking news bullets for better presentation
   * @param {string[]} bullets - Raw breaking news bullets
   * @returns {Promise<string[]>} Organized bullets
   */
  async organizeBreakingBullets(bullets) {
    if (!this.canMakeCall() || bullets.length <= 1) {
      return bullets.map(bullet => this.enforceFormat(bullet, 'breaking'));
    }
    
    const systemPrompt = `You are an NFL content organizer. Take these breaking news bullets and organize them for better readability. Group similar topics together, ensure consistent formatting, and improve clarity while preserving all facts. Return each bullet on a new line.`;
    
    const userPrompt = `Organize these NFL breaking news bullets for better presentation:

${bullets.map((b, i) => `${i + 1}. ${b}`).join('\n')}`;
    
    const organizedBullets = await this.makeGPTCall(systemPrompt, userPrompt);
    
    if (organizedBullets.length > 0) {
      console.log(`   📋 Organized ${bullets.length} breaking bullets with GPT`);
      return organizedBullets.map(bullet => this.enforceFormat(bullet, 'breaking'));
    }
    
    // Fallback to original bullets if organize fails
    return bullets.map(bullet => this.enforceFormat(bullet, 'breaking'));
  }
