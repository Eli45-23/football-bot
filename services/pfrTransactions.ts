...
class PFRTransactionsService {
  ...
  private transactionsFeedUrl: string;
  private seenUrls: Set<string>;
  private ROSTER_PATTERNS: RegExp;
   ...
   * @returns Promise<FormattedTransactions> Formatted transactions with pagination
   */
  async fetchTransactions(lookbackHours: number = 24): Promise<FormattedTransactions> {
    try {
   ...
   */
  private async processTransactionItem(item: any): Promise<TransactionData | null> {
   ...
   */
  private formatBulletsExact(transactions: TransactionData[]): string[] {
   ...
   */
  private extractPlayerName(title: string): string | null {
    // Look for capitalized names (First Last, First Last Jr., etc.)
    const namePatterns = [
      /\b([A-Z][a-z]+(?:\s+[A-Z][a-z]*\.?)*\s+[A-Z][a-z]+(?:\s+(?:Jr|Sr|III|IV))?)(?=\s|,|$)/,
      /\b([A-Z]\.\s*[A-Z][a-z]+)(?=\s|,|$)/,
      /\b([A-Z][a-z]+\s+[A-Z]'[A-Z][a-z]+)(?=\s|,|$)/
    ];
   ...
   */
  private extractTeamName(title: string): string | null {
   ...
   */
  private extractExtraDetails(content: string, player: string, team: string, action: string): string | undefined {
   ...
   */
  private getTeamAbbreviation(teamName: string): string | null {
   ...
   */
  private sentenceCase(action: string): string {
    if (!action) return '';
...
export default new PFRTransactionsService();
